...
test_that("sigma cannot be negative", {
    expect_error(make_data(n = 10, p = 3, q = 2, sigma = -10^17))
})


test_that("rho_x cannot be negative", {
    expect_error(make_data(n = 10, p = 3, q = 2, rho_x = -10^17))
})
...
test_that("rho_err cannot be negative", {
    expect_error(make_data(n = 10, p = 3, q = 2, rho_err = -10^17))
})
...
test_that("zero_appeal cannot be negative", {
    expect_error(make_data(n = 10, p = 3, q = 2, zero_appeal = -10^17))
})

test_that("n_edge cannot be negative", {
    expect_error(make_data(n = 10, p = 3, q = 2, n_edge = -10^17))
})

test_that("min_ev cannot be negative", {
    expect_error(make_data(n = 10, p = 3, q = 2, min_ev = -10^17))
})
...
data = make_data(n = 5, p = 3, q = 2, b1 = sqrt(0.5),
...
test_that("At least one entry labeled 'X' in returned output lists is
          ...
          (5,3,2)", {
              test_mat_1 = matrix( c( 0.150,  0.067,  0.169,
                                      0.699, -1.616, -0.892,
                                      0.970, -0.289, -0.012,
                                      0.112, -0.257, -0.585,
                                      0.800,  0.106,  0.318),
                                   5, 3, byrow = T)
              expect_equal(round(data[[1]]$X,3), test_mat_1)
              })

test_that("At least one entry labeled 'Y' in returned output lists is
          what it is expected to be for problem size (n,p,q) =
          (5,3,2)", {
              test_mat_2 = matrix( c( 0.382,  0.000,
                                      -0.116, -1.373,
                                      1.180, -0.359,
                                      -0.557, -0.077,
                                      1.346, -0.060),
                                   5, 2, byrow = T)
              expect_equal(round(data[[1]]$Y,3), test_mat_2)
              })

test_that("At least one entry labeled 'B' in returned output lists is
          what it is expected to be for problem size (n,p,q) =
          (5,3,2)", {
              test_mat_3 = matrix( c( 0.215, 0,
                                      0.000, 0,
                                      1.086, 0),
                                   3, 2, byrow = T)
              expect_equal(round(data[[1]]$B,3), test_mat_3)
              })

test_that("At least one entry labeled 'E' in returned output lists is
          what it is expected to be for problem size (n,p,q) =
          (5,3,2)", {
              test_mat_4 = matrix( c( 0.166,  0.000,
                                      0.703, -1.373,
                                      0.985, -0.359,
                                      0.054, -0.077,
                                      0.828, -0.060),
                                   5, 2, byrow = T)
              expect_equal(round(data[[1]]$E,3), test_mat_4)
              })

test_that("At least one entry labeled 'Sigma' in returned output lists is
          what it is expected to be for problem size (n,p,q) =
          (5,3,2)", {
              test_mat_5 = matrix( c( 1.0, 0.7,
                                      0.7, 1.0),
                                   2, 2, byrow = T)
              expect_equal(round(data[[1]]$Sigma,3), test_mat_5)
              })

test_that("At least one entry labeled 'Omega' in returned output lists is
          what it is expected to be for problem size (n,p,q) =
          (5,3,2)", {
              test_mat_6 = matrix( c(  1.961, -1.373,
                                       -1.373,  1.961),
                                   2, 2, byrow = T)
              expect_equal(round(data[[1]]$Omega,3), test_mat_6)
              })

test_that("At least one entry labeled 'Sigma_x' in returned output lists is
          what it is expected to be for problem size (n,p,q) =
          (5,3,2)", {
              test_mat_7 = matrix( c( 1.00, 0.6, 0.36,
                                      0.60, 1.0, 0.60,
                                      0.36, 0.6, 1.00),
                                   3, 3, byrow = T)
              expect_equal(round(data[[1]]$Sigma_x,3), test_mat_7)
              })
